import { safeExec, ExecResult } from "../exec";
...
describe("safeExec", () => {
  ...
  it("resolves with stdout/stderr on success", async () => {
    const result = await safeExec(echoCmd);
  ...
  it("never throws — returns stderr on failure", async () => {
    const result = await safeExec("nonexistent_command_that_does_not_exist_12345", {
      timeout: 5000,
    });
  ...
  it("accepts custom timeout option", async () => {
    ...
    const result = await safeExec(echoCmd, { timeout: 30000 });
    expect(result.stdout.trim()).toBe("hello");
  });
  ...
  it("uses correct shell for platform", async () => {
    ...
    if (process.platform === "win32") {
      ...
      const result = await safeExec("$PSVersionTable.PSVersion.ToString()");
      expect(result.stdout.trim().length).toBeGreaterThan(0);
    } else {
      // Shell-specific: $SHELL exists in /bin/sh
      const result = await safeExec("echo $SHELL");
      expect(result.stdout.trim().length).toBeGreaterThan(0);
    }
  });
});
